...
/**
 * JUnit test for {@link VBridgeIfIdentifier}.
 */
public class VBridgeIfIdentifierTest extends TestBase {
    /**
     * Root XML element name associated with {@link VBridgeIfIdentifier}
     * class.
     */
    private static final String  XML_ROOT = "vbridge-if-path";

    /**
     * Return a list of {@link XmlDataType} instances that specifies XML node
     * types mapped to a {@link VBridgeIfIdentifier} instance.
     *
    ...
    /**
     * Test case for
     * {@link VBridgeIfIdentifier#create(String, String, String, boolean)}.
     *
    ...
    public void testCreate() throws Exception {
        ...
        for (boolean find: bools) {
            try {
                VBridgeIfIdentifier.
                    create((String)null, "vbr", "vif", find);
                unexpected();
        ...
        for (boolean find: bools) {
            try {
                VBridgeIfIdentifier.
                    create("vtn", "vbr", (String)null, find);
                unexpected();
            } catch (RpcException e) {
                assertEquals(etag, e.getErrorTag());
                assertEquals(vtag, e.getVtnErrorTag());
        ...
        for (boolean find: bools) {
            ...
            try {
                VBridgeIfIdentifier.create("", "vbr", "vif", find);
                unexpected();
            } catch (RpcException e) {
                assertEquals(etag, e.getErrorTag());
                assertEquals(vtag, e.getVtnErrorTag());
            ...
            try {
                VBridgeIfIdentifier.create("vtn", "", "vif", find);
                unexpected();
            } catch (RpcException e) {
                assertEquals(etag, e.getErrorTag());
                assertEquals(vtag, e.getVtnErrorTag());
            ...
            try {
                VBridgeIfIdentifier.create(badName, "vbr", "vif", find);
                unexpected();
            } catch (RpcException e) {
                assertEquals(etag, e.getErrorTag());
                assertEquals(vtag, e.getVtnErrorTag());
            ...
            try {
                VBridgeIfIdentifier.create("vtn", badName, "vif", find);
                unexpected();
            } catch (RpcException e) {
                assertEquals(etag, e.getErrorTag());
                assertEquals(vtag, e.getVtnErrorTag());
        ...
        try {
            VBridgeIfIdentifier.create("vtn", "vbr", "", true);
            unexpected();
        } catch (RpcException e) {
            assertEquals(etag, e.getErrorTag());
            assertEquals(vtag, e.getVtnErrorTag());
        ...
        try {
            VBridgeIfIdentifier.create("vtn", "vbr", badName, true);
            unexpected();
        } catch (RpcException e) {
            assertEquals(etag, e.getErrorTag());
            assertEquals(vtag, e.getVtnErrorTag());
        ...
        try {
            VBridgeIfIdentifier.create("vtn", "vbr", "", false);
            unexpected();
        } catch (RpcException e) {
            assertEquals(etag, e.getErrorTag());
            assertEquals(vtag, e.getVtnErrorTag());
        ...
        try {
            VBridgeIfIdentifier.create("vtn", "vbr", badName, false);
            unexpected();
        } catch (RpcException e) {
            assertEquals(etag, e.getErrorTag());
            assertEquals(vtag, e.getVtnErrorTag());
    ...
    public void testGetTenantName() {
        ...
        for (String name: names) {
            ...
            ifId = new VBridgeIfIdentifier(vbrId, iname);
            assertEquals(tname, ifId.getTenantName());
            assertEquals(name, ifId.getTenantNameString());
        }
    }
     ...
     * {@link VBridgeIfIdentifier#getIdentifierBuilder()}.
     */
    @Test
    ...
    public void testContains() {
        ...
        for (VnodeName tname: tnames) {
            ...
            for (VnodeName bname: bnames) {
                ...
                for (VnodeName iname: inames) {
                    VBridgeIfIdentifier ifId1 =
                        new VBridgeIfIdentifier(tname, bname, iname);
                    VBridgeIfIdentifier ifId2 =
                        new VBridgeIfIdentifier(tname, bname, iname);
    ...
    public void testContainsVirtualNodePath() {
        // VBridgeIfIdentifier.contains(VirtualNodePath) returns true only if
        ...
        for (VnodeName vtnName: tnames) {
            ...
            for (VnodeName brName: bnames) {
                ...
                for (VnodeName ifName: inames) {
                    VBridgeIfIdentifier ifId =
                        new VBridgeIfIdentifier(vtnName, brName, ifName);
                    ...
                    VirtualNodePath vmpath = new VirtualNodePathBuilder().
                        ...
                        addAugmentation(BridgeMapInfo.class, bmi).
                        build();
                    Collections.addAll(falseList, ipath2, vmpath);
                    assertEquals(true, ifId.contains(ipath1));

                    for (VirtualNodePath vpath: falseList) {
                        assertEquals(false, ifId.contains(vpath));
                    }

                    falseList.add(ipath1);
                }
            }
        }
    }
    ...
    /**
     * Test case for {@link VBridgeIfIdentifier#getBridgeIdentifier()}.
     */
    @Test
        ...
        for (VnodeName tname: tnames) {
            for (VnodeName bname: bnames) {
                ...
                for (VnodeName iname: inames) {
                    VBridgeIfIdentifier ifId =
                        new VBridgeIfIdentifier(tname, bname, iname);
                    VBridgeIdentifier ident = ifId.getBridgeIdentifier();
    ...
    /**
     * Test case for {@link VBridgeIfIdentifier#getType()}.
     */
    @Test
    ...
    /**
     * Test case for {@link VBridgeIfIdentifier#getVNodeIdentifier()}.
     */
    @Test
            ...
            for (VnodeName bname: bnames) {
                for (VnodeName iname: inames) {
                    VBridgeIfIdentifier ifId =
                        new VBridgeIfIdentifier(tname, bname, iname);
                    for (int i = 0; i < 5; i++) {
                        VBridgeIfIdentifier ident = ifId.getVNodeIdentifier();
    ...
    /**
     * Test case for {@link VBridgeIfIdentifier#replaceTenantName(VnodeName)}.
     */
    @Test
    ...
    /**
     * Test case for {@link VBridgeIfIdentifier#toRedirectDestination()}.
     */
    @Test
    public void testToRedirectDestination() {
        ...
        for (String tname: tnames) {
            ...
            for (String bname: bnames) {
                VnodeName vbrName = new VnodeName(bname);
                for (String iname: inames) {
                    VnodeName ifName = new VnodeName(iname);
                    VBridgeIfIdentifier ifId =
                        new VBridgeIfIdentifier(vtnName, vbrName, ifName);
                    RedirectDestination rd = ifId.toRedirectDestination();
                    assertEquals(null, rd.getTenantName());
                    assertEquals(bname, rd.getBridgeName());
                    assertEquals(null, rd.getTerminalName());
        ...
        for (String tname: tnames) {
            ...
            for (String bname: bnames) {
                ...
                for (String iname: inames) {
                    ...
                    VBridgeIfIdentifier ifId1 =
                        new VBridgeIfIdentifier(vtnName1, vbrName1, ifName1);
                    VBridgeIfIdentifier ifId2 =
                        new VBridgeIfIdentifier(vtnName2, vbrName2, ifName2);
            ...
            for (String bname: bnames) {
                ...
                for (String iname: inames) {
                    ...
                    String expected = joinStrings(
                        "vBridge-IF:", null, "/", tnameStr, bnameStr,
                        inameStr);
    ...
    /**
     * Ensure that {@link VBridgeIfIdentifier} is mapped to XML root element.
     *
     * @throws Exception  An error occurred.
     */
    @Test
    public void testJAXB() throws Exception {
        Marshaller[] marshallers = {
            ...
            createMarshaller(VBridgeIfIdentifier.class),
        };
        Unmarshaller[] unmarshallers = {
            createUnmarshaller(VNodeIdentifier.class),
            createUnmarshaller(VInterfaceIdentifier.class),
            createUnmarshaller(VBridgeIfIdentifier.class),
        };

        Class<VBridgeIfIdentifier> type = VBridgeIfIdentifier.class;
        String[] tnames = {null, "vtn", "vtn_1"};
        String[] bnames = {null, "vbr", "vbridge_1"};
        String[] inames = {null, "if", "vif_1"};
        for (String tname: tnames) {
            for (String bname: bnames) {
                for (String iname: inames) {
                    ...
                    for (Unmarshaller um: unmarshallers) {
                        VBridgeIfIdentifier ifId = unmarshal(um, xml, type);
                        assertEquals(tname, ifId.getTenantNameString());
                        assertEquals(bname, ifId.getBridgeNameString());
                        assertEquals(iname, ifId.getInterfaceNameString());
